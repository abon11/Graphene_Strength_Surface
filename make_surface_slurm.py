...
def build_job_command(base_args, x_erate, y_erate, cores):
    return [
        "mpiexec", "-n", str(cores), 
        "python3", "one_sim.py",
...
def main():
    ...
    for i in range(len(x_rates)):
        job_queue.append(build_job_command(args, x_rates[i], y_rates[i], MIN_CORES_PER_JOB))
        if str2bool(args.both_directions) and x_rates[i] != y_rates[i]:
            job_queue.append(build_job_command(args, y_rates[i], x_rates[i], MIN_CORES_PER_JOB))
