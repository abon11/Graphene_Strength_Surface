...
SHEET_PATH="${SHEET_PATH:-/hpc/home/avb25/Graphene_Strength_Surface/simulation_data/data_files/data.60_60_rel_airebolight}"
X_ATOMS="${X_ATOMS:-60}"
Y_ATOMS="${Y_ATOMS:-60}"
...
SIM_LENGTH="${SIM_LENGTH:-20000000}"
...
STORAGE_PATH="${STORAGE_PATH:-/hpc/dctrl/avb25/Graphene_Strength_Surface/simulation_data/rebo_uniaxial}"
POTENTIAL="${POTENTIAL:-"airebo_light"}"

submit_simulation() {
    local repeat_sim=$1

    if [ -n "$repeat_sim" ]; then
        sbatch "$SLURM_SCRIPT" "$CORES_PER_JOB" "$SHEET_PATH" "$X_ATOMS" "$Y_ATOMS" "$DEFECTS" "$DEFECT_RANDOM_SEED" "$SIM_LENGTH" "$TIMESTEP" "$THERMO" "$DETAILED_DATA" "$THETA" "$FRACTURE_WINDOW" "$STORAGE_PATH" "$ACCEPT_DUPES" "$ANGLE_TESTING" "$x_erate" "$y_erate" "$xy_erate" "$POTENTIAL" "$repeat_sim"
        echo "REPEATING SIM $repeat_sim."
    else
        sbatch "$SLURM_SCRIPT" "$CORES_PER_JOB" "$SHEET_PATH" "$X_ATOMS" "$Y_ATOMS" "$DEFECTS" "$DEFECT_RANDOM_SEED" "$SIM_LENGTH" "$TIMESTEP" "$THERMO" "$DETAILED_DATA" "$THETA" "$FRACTURE_WINDOW" "$STORAGE_PATH" "$ACCEPT_DUPES" "$ANGLE_TESTING" "$x_erate" "$y_erate" "$xy_erate" "$POTENTIAL"
        echo "Starting sheet with: x_erate=$x_erate, y_erate=$y_erate, xy_erate=$xy_erate, SHEET_PATH=$SHEET_PATH, X_ATOMS=$X_ATOMS, Y_ATOMS=$Y_ATOMS, DEFECTS=$DEFECTS, DEFECT_RANDOM_SEED=$DEFECT_RANDOM_SEED, ACCEPT_DUPES=$ACCEPT_DUPES, SIM_LENGTH=$SIM_LENGTH, TIMESTEP=$TIMESTEP, THERMO=$THERMO, DETAILED_DATA=$DETAILED_DATA, ANGLE_TESTING=$ANGLE_TESTING, THETA=$THETA, FRACTURE_WINDOW=$FRACTURE_WINDOW, STORAGE_PATH=$STORAGE_PATH, POTENTIAL=$POTENTIAL"
...
# SHEET_PATH="${SHEET_PATH:-/hpc/home/avb25/projects/Graphene_Strength_Surface/data_files/data.60_60_rel1}"
# submit_simulation
