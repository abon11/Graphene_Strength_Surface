...
def main():
    pairs = [(130, 20), (130, 150), (140, 20), (150, 20), (150, 40)]
    for n_atoms_x, n_atoms_y in pairs:
        ...
        dims = calc_dims(n_atoms_x, n_atoms_y, box_tol=box_tol, z_tol=z_tol)

        # print("Input name for data file (will be saved as data.<name>)\n")
        # filename = input()
        filename = f"{n_atoms_x}_{n_atoms_y}"
        filepath = f'{local_config.DATA_DIR}/data_files/data.{str(filename)}'
...
def calc_dims(nx, ny, box_tol=0.0, z_tol=0.0, carbon_distance=1.42):
    dims = np.zeros((6))
    lattice_a = carbon_distance * 2

    # Box dimensions
    dims[0] = (nx / 2 - 0.5) * (carbon_distance + lattice_a) + box_tol
    dims[1] = (ny / 2) * (carbon_distance * np.sqrt(3)) + box_tol
