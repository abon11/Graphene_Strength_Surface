...
def main():
    ...
    parser.add_argument("--fourier_order", type=int, required=True)
    parser.add_argument("--lam", type=float, required=True)
    parser.add_argument("--defs", type=str, required=True)
    args = parser.parse_args()
    fourier_order = args.fourier_order
    lam = args.lam
    defs = args.defs.lower()
    ...
    save_fits_to = f'z_{defs}{fourier_order}_reg{lam_str}.csv'

    if rank == 0:
        print(f"Running {defs} on {size} MPI ranks...")
    ...
    for idx in local_indices:
        ...
        surface = Surface(group_df, interest_value, fourier_order, reg=lam)
        ...
        try:
            local_results.append({
                ...
                **{f"z{i}": surface.zs[i] for i in range(len(surface.zs))},
                ...
                "NRMSE": float(stats["nrmse"]),
                "norm_z": float(stats["norm_z"])
            })
            print(f"[Rank {rank}] Finished seed {int(instance)}")
        except TypeError:
            print(f"[Rank {rank}] Not adding this seed {int(instance)} to the list ##############")

    # Each rank writes its own CSV
    local_df = pd.DataFrame(local_results)
    local_df.to_csv(f"{save_fits_to}_rank{rank}.csv", index=False)
...
class Surface():
    def __init__(self, points, interest_value, fourier_order, reg=0):
        """
        ...
        self.points = points
        self.mean = np.mean(points["Strength_1"].to_numpy())
        self.reg = reg
    ...
    def loss(self, params, return_resid=False, N=4, lam=0):
        """
        ...
        if return_resid:
            ...
            # we want to regularize the loss when minimizing to penalize high frequency modes
            reg = lam * 0.5 *  np.sum(params**2)
    ...
    def fit_drucker_prager(self):
        ...
        try:
            ...
            # else:
            result = minimize(
                lambda p: self.loss(p, N=N, lam=self.reg),
                x0=initial_guess,
                method='L-BFGS-B',
    ...
    def compute_loss_statistics(self, print_outputs=False):
        ...
        if print_outputs:
            ...
            print(f"zs = {self.zs}")
            print(f"NRMSE: {nrmse:.4f}, Max Residual: {max_residual:.4f}, Total Loss: {total_loss:.4f}.")
