...
def main():
    ...
    fig, ax = plt.subplots(figsize=(3.56, 3.56))
    ...
    for instance, group_df in grouped:
        ...
        if instance == 0:
            surface.plot_onto_ax(ax, 'red', 'AC')
        else:
            surface.plot_onto_ax(ax, 'blue', 'ZZ')
    ...
    folder = f"{local_config.DATA_DIR}/rotation_tests"
    fullpath = f"{folder}/plots/{save_fits_to[:-4]}.pdf"
    fig.tight_layout()
...
class Surface():
    ...
    def plot_onto_ax(self, ax, color, lab, resolution=1000):
        ...
        ax.contour(sig1, sig2, F, levels=[0], colors=color, linewidths=1, alpha=0.8)
        ax.plot([], [], color=color, label=f"Strength surface - {lab}")  # for legend

        # Plot data points
        ax.scatter(sig1_vals, sig2_vals, c=color, label=f"MD points - {lab}", alpha=0.8, s=10)
        # ax.scatter(sig2_vals, sig1_vals, c='blue', alpha=0.2)

        ax.plot([-50, 135], [0, 0], color='black', lw=0.75)
        ax.plot([0, 0], [-50, 135], color='black', lw=0.75)
